...
def split_data(csv_files_names: list):
    ...
    for city in CITIES:
        ...
        city_listings_path = [
            x for x in csv_files_names if x.startswith("listings") and city in x][0]
        city_reviews_path = [
        ...
        data_city = city_listings.rename({"id": "listing_id"}, axis=1).merge(
            city_reviews, on="listing_id", how="inner")
        ...
        train, test = train_test_split(
            data_city, test_size=0.2, shuffle=True, stratify=data_city["polarity_class"])
    ...
    if upload:
        if is_dataset_new:
            ...
            api.dataset_create_version(
                folder="./dataset", version_notes="update")
